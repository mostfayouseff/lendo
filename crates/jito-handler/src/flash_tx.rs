...
// Clean Atomic Solend Flash Loan Builder (Final Version)
// 
// Fixed: ATA_PROGRAM_ID added, unused variables removed, warnings cleaned.
// Accepts swap_data as Vec<(program_id, instruction_data)> from your trade.
// =============================================================================
...
const ATA_PROGRAM_ID: &str = "ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJe8bv";   // ← Added
...
pub fn build_flash_loan_tx(
    ...
    let mut accounts: Vec<[u8; 32]> = vec![
        operator,                                      // 0: Operator (signer + writable)
        b58_to_32(SOLEND_SOL_LIQ_SUPPLY),              // 1
        borrower_wsol_ata,                             // 2
        b58_to_32(SOLEND_SOL_RESERVE),                 // 3
        b58_to_32(SOLEND_FEE_RECEIVER),                // 4
        lending_mkt_authority,                         // 5
    ];
    ...
    let mut dex_indices: std::collections::HashMap<String, u8> = std::collections::HashMap::new();
    ...
    // ── Instructions ─────────────────────────────────────────────────────────
    let mut instructions: Vec<(u8, Vec<u8>, Vec<u8>)> = Vec::new(); // (program_idx, account_idxs, data)
    ...
    instructions.push((
        solend_idx,
        vec![1, 2, 3, 4, 4, lending_market_idx, 5, token_idx, sysvar_idx],
        borrow_data,
    ));
    ...
    for (prog_id, data) in swap_data {
        ...
        instructions.push((prog_idx, vec![0, 2], data.clone())); // operator + wsol_ata
    }

    // 3. Flash Repay
    let mut repay_data = vec![REPAY_DISCRIMINATOR];
    repay_data.extend_from_slice(&repay_lamports.to_le_bytes());
    repay_data.push(0); // borrow instruction index = 0

    instructions.push((
        solend_idx,
        vec![2, 1, 3, 4, 4, lending_market_idx, 5, 0, token_idx, sysvar_idx],
        repay_data,
    ));

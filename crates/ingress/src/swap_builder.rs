...
// 
// Fixes applied:
// • Fixed E0382: moved `resp` after .text().await in CoinGecko
// • Better rate-limit handling and backoff
// • Reduced polling to avoid 429s
// =============================================================================
...
const POLL_INTERVAL_MS: u64 = 10_000;           // 10 seconds - helps avoid 429
...
impl JupiterMonitor {
    ...
    async fn try_failover(
        ...
        match next_result {
            Ok(prices) if !prices.is_empty() => {
                info!(from = current.name(), to = next.name(), "NEW API DISCOVERED AND VERIFIED: switching to {}", next.name());
                for (mint, price) in &prices {
                    price_cache.insert(mint.clone(), *price);
                }
    ...
    // FIXED: Check status BEFORE calling .text()

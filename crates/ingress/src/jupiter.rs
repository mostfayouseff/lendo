...
//
// Flow:
//   1. GET /v2/quote        → fetch best route quote
//   2. POST /v2/swap-instructions → fetch individual instructions with full
...
const SWAP_INSTRUCTIONS_URL: &str = "https://lite-api.jup.ag/swap/v1/swap-instructions";
...
pub async fn build_signed_swap_transaction(
    ...
    let quote = get_quote(client, input_mint, output_mint, amount, slippage_bps, api_key)
        .await
        .context("Jupiter /v2/quote failed")?;
    ...
    info!(
        ...
        "Jupiter /v2/swap-instructions: instructions received"
    );
...
async fn get_quote(
    ...
    debug!(url = %url, "Fetching Jupiter /v2/quote");
...
async fn get_swap_instructions(
    ...
    debug!(url = SWAP_INSTRUCTIONS_URL, "Posting to Jupiter /v2/swap-instructions");
    ...
    if !status.is_success() {
        ...
        return Err(anyhow::anyhow!(
            "Jupiter /v2/swap-instructions HTTP {}: {}",
            status,
